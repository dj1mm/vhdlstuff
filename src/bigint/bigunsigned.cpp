...
// Memory management definitions have moved to the bottom of number_like_array.h.

// The templates used by these constructors and converters are at the bottom of
// bigunsigned.h.

bigunsigned::bigunsigned(unsigned long  x) { init_from_primitive      (x); }
bigunsigned::bigunsigned(unsigned int   x) { init_from_primitive      (x); }
bigunsigned::bigunsigned(unsigned short x) { init_from_primitive      (x); }
bigunsigned::bigunsigned(         long  x) { init_from_signed_primitive(x); }
bigunsigned::bigunsigned(         int   x) { init_from_signed_primitive(x); }
bigunsigned::bigunsigned(         short x) { init_from_signed_primitive(x); }

unsigned long  bigunsigned::to_unsigned_long () const { return convert_to_primitive      <unsigned long >(); }
unsigned int   bigunsigned::to_unsigned_int  () const { return convert_to_primitive      <unsigned int  >(); }
unsigned short bigunsigned::to_unsigned_short() const { return convert_to_primitive      <unsigned short>(); }
long           bigunsigned::to_long         () const { return convert_to_signed_primitive<         long >(); }
int            bigunsigned::to_int          () const { return convert_to_signed_primitive<         int  >(); }
short          bigunsigned::to_short        () const { return convert_to_signed_primitive<         short>(); }

// BIT/BLOCK ACCESSORS

void bigunsigned::set_block(index_t i, blk_t new_block) {
    if (new_block == 0) {
        if (i < len) {
            blk[i] = 0;
            zap_leading_zeros();
        ...
        if (i >= len) {
            ...
            allocate_and_copy(i+1);
            // Zero any added blocks that we aren't setting.
            for (index_t j = len; j < i; j++)
                blk[j] = 0;
            len = i+1;
        }
        blk[i] = new_block;
 ...
 * proper scope. */
bigunsigned::index_t bigunsigned::bit_length() const {
    if (is_zero())
        return 0;
    else {
        blk_t leftmostBlock = get_block(len - 1);
        index_t leftmostBlockLen = 0;
...
void bigunsigned::set_bit(index_t bi, bool newBit) {
    index_t blockI = bi / N;
    blk_t block = get_block(blockI), mask = blk_t(1) << (bi % N);
    block = newBit ? (block | mask) : (block&  ~mask);
    set_block(blockI, block);
}

// COMPARISON
bigunsigned::comparison_result bigunsigned::compare_to(const bigunsigned& x) const {
...
void bigunsigned::subtract(const bigunsigned& a, const bigunsigned& b) {
    ...
    if (b.len == 0) {
        ...
        throw "Negative result in unsigned calculation";
    // Some variables...
 ...
 * Programming'' (replace `place' by `blk_t'):
 *
 ...
 *
 * `get_shifted_block' returns the `x'th block of `num << y'.
 ...
 */
inline bigunsigned::blk_t get_shifted_block(const bigunsigned& num,
    bigunsigned::index_t x, unsigned int y) {
    bigunsigned::blk_t part1 = (x == 0 || y == 0) ? 0 : (num.blk[x - 1] >> (bigunsigned::N - y));
    bigunsigned::blk_t part2 = (x == num.len) ? 0 : (num.blk[x] << y);
    return part1 | part2;
}
...
void bigunsigned::multiply(const bigunsigned& a, const bigunsigned& b) {
    ...
    for (i = 0; i < a.len; i++) {
        ...
        for (i2 = 0; i2 < N; i2++) {
            if ((a.blk[i] & (blk_t(1) << i2)) == 0)
                continue;
            /*
             * Add b to this, shifted left i blocks and i2 bits.
             * j is the index in b, and k = i + j is the index in this.
             *
             * `get_shifted_block', a short inline function defined above,
            ...
            for (j = 0, k = i, carryIn = false; j <= b.len; j++, k++) {
                 ...
                 */
                temp = blk[k] + get_shifted_block(b, j, i2);
                carryOut = (temp < blk[k]);
                if (carryIn) {
                    temp++;
                    carryOut |= (temp == 0);
 ...
 */
void bigunsigned::divide_with_remainder(const bigunsigned& b, bigunsigned& q) {
    ...
    if (this == &q)
        throw "Cannot write quotient and remainder into the same variable";
    ...
    /*
     * If *this.len < b.len, then *this < b, and we can be sure that b doesn't
     * go into *this at all.  The quotient is 0 and *this is already the
     * remainder (so leave it alone).
     */
    if (len < b.len) {
        q.len = 0;
        return;
    ...
    while (i > 0) {
        ...
        // (Remember, N is the number of bits in a blk_t.)
        q.blk[i] = 0;
        i2 = N;
        while (i2 > 0) {
            i2--;
 ...
 * the output length and the necessity of zap_leading_zeros. */

void bigunsigned::bit_and(const bigunsigned& a, const bigunsigned& b) {
    DTRT_ALIASED(this == &a || this == &b, bit_and(a, b));
...
void bigunsigned::bit_or(const bigunsigned& a, const bigunsigned& b) {
    DTRT_ALIASED(this == &a || this == &b, bit_or(a, b));
    index_t i;
    const bigunsigned* a2, *b2;
    if (a.len >= b.len) {
        a2 = &a;
    ...
    for (; i < a2->len; i++)
        blk[i] = a2->blk[i];
    len = a2->len;
    // Doesn't need zap_leading_zeros.
}

void bigunsigned::bit_xor(const bigunsigned& a, const bigunsigned& b) {
    DTRT_ALIASED(this == &a || this == &b, bit_xor(a, b));
    index_t i;
    const bigunsigned* a2, *b2;
    if (a.len >= b.len) {
        a2 = &a;
...
void bigunsigned::bit_shift_left(const bigunsigned& a, int b) {
    DTRT_ALIASED(this == &a, bit_shift_left(a, b));
    if (b < 0) {
        if (b << 1 == 0)
            throw "Pathological shift amount not implemented";
        else {
            bit_shift_right(a, -b);
            return;
        }
    }
    index_t shiftBlocks = b / N;
    ...
    for (j = 0, i = shiftBlocks; j <= a.len; j++, i++)
        blk[i] = get_shifted_block(a, j, shiftBits);
    // Zap possible leading zero
    if (blk[len - 1] == 0)
        len--;
}

void bigunsigned::bit_shift_right(const bigunsigned& a, int b) {
    DTRT_ALIASED(this == &a, bit_shift_right(a, b));
    if (b < 0) {
        if (b << 1 == 0)
            throw "Pathological shift amount not implemented";
        else {
    ...
    // within each block lets us use get_shifted_block.
    index_t rightShiftBlocks = (b + N - 1) / N;
    ...
    for (j = rightShiftBlocks, i = 0; j <= a.len; j++, i++)
        blk[i] = get_shifted_block(a, j, leftShiftBits);
    // Zap possible leading zero
    if (blk[len - 1] == 0)
        len--;
}
...
void bigunsigned::operator++() {
    ...
    if (carry) {
        // Allocate and then increase length, as in divide_with_remainder
        allocate_and_copy(len + 1);
        len++;
        blk[i] = 1;
    }
...
void bigunsigned::operator--() {
    if (len == 0)
        throw "Cannot decrement an unsigned zero";
    index_t i;
    bool borrow = true;
    for (i = 0; borrow; i++) {
        borrow = (blk[i] == 0);
